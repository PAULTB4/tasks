...
export function CreateCategoryDialog({
  ...
  const selectedTypeLabel = selectedType === 'folder' ? 'carpeta' : 'lista'
  ...
  return (
    <Dialog
      open={open}
      onClose={handleClose}
      title={parentId ? 'Crear dentro de esta carpeta' : 'Crear nuevo espacio'}
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3 justify-end border-t border-surface-200 pt-4 dark:border-surface-800">
          ...
          <Button type="submit" disabled={isSubmitting || !name.trim()}>
            {isSubmitting ? 'Creando...' : `Crear ${selectedTypeLabel}`}
          </Button>
        </div>
      </form>
    </Dialog>
